...
from validate import validate_output_pydantic, OutputData
...
with open(ipid_path, "rb") as f:
    data = f.read()

schema_str = json.dumps(OutputData.model_json_schema(), indent=4)

# Write an prompt to accurately extract ONLY the specified fields (see README) from the PDF
prompt = f"""
Act as an insurance expert reviewing this Insurance Product Information Document (IPID).
Your task is to extract information *strictly* matching the provided JSON schema below.
Output *only* a JSON object.
This JSON object *must* contain *only* the fields defined in the `properties` section of the following schema.
*Do not* include any other fields, keys, or extraneous text, even if they seem relevant in the document.
Adhere strictly to the types specified (e.g., boolean).

<schema>
{schema_str}
</schema>

Generate the JSON output directly.
"""

# Generate Response
response = generate(prompt, data, response_mime_type="application/json")
